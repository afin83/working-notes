...
const applyMode = (selectedMode: 'auto' | 'light' | 'dark') => {
  ...
  if (selectedMode === 'auto') {
    ...
    // Check system preference and add appropriate class for consistency
...
// Helper function to check if route matches nav item
const isActiveNavItem = (item: NavItem): boolean => {
  if (item.to) {
    return route.path === item.to
  }
  // For items with children, check if current route matches any child exactly
  if (item.children) {
...
// Get current submenu items based on view
const currentSubmenuItems = computed(() => {
  const item = navigationConfig.find((item) => item.label.toLowerCase() === currentView.value)
...
<template>
  <nav class="border-b" aria-label="Main navigation">
    <div
      class="flex items-center mx-auto max-w-screen-2xl py-4 sm:py-7 px-5 sm:px-6 md:px-8 xl:px-10"
    >
      <!-- Logo/Brand -->
      <RouterLink
        to="/"
        class="flex items-center gap-2 hover:opacity-80 transition-opacity"
        aria-label="Working Notes home"
      >
        <svg
          class="h-11 sm:h-14 fill-primary"
          viewBox="0 0 99 56"
          xmlns="http://www.w3.org/2000/svg"
          role="img"
          aria-label="Working Notes logo"
        >
          <path
      ...
      <div class="hidden md:flex items-center flex-1 ml-8">
        ...
        <NavigationMenu>
          <NavigationMenuList>
            <NavigationMenuItem v-for="item in navigationConfig" :key="item.label">
              <!-- Simple link (no children) -->
              <NavigationMenuLink v-if="!item.children" as-child>
                <RouterLink
                  :to="item.to!"
                  :class="[
                    'group inline-flex h-9 w-max items-center justify-center rounded-md px-4 py-2 text-base font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none',
                    isActiveNavItem(item) ? 'bg-accent text-accent-foreground' : 'bg-background',
                  ]"
                >
                  {{ item.label }}
                </RouterLink>
              ...
              <DropdownMenu v-else v-model:open="dropdownStates[item.label]">
                <DropdownMenuTrigger as-child>
                  <button
                    ...
                    :aria-expanded="dropdownStates[item.label] || false"
                    aria-haspopup="menu"
                ...
                <DropdownMenuContent align="start" class="w-[250px]" role="menu">
                  <DropdownMenuItem
                    ...
                    role="menuitem"
                  >
                    <RouterLink :to="child.to" class="cursor-pointer">
                      <div>
                        <div class="font-medium text-base">{{ child.label }}</div>
                        <div v-if="child.description" class="text-sm text-muted-foreground">
                          {{ child.description }}
                        </div>
                      </div>
                    </RouterLink>
      ...
      <Sheet v-model:open="mobileMenuOpen" @update:open="handleSheetChange">
        ...
        <SheetContent
          side="right"
          class="w-[300px] sm:w-[400px] overflow-hidden"
          hide-close
          id="mobile-navigation-menu"
        >
          <!-- Custom header with back button (submenu) or close button (main) -->
          <div class="flex items-center justify-between p-3 border-b h-21">
            <button
              v-if="currentView !== 'main'"
              @click="goBack"
              class="flex items-center gap-2 text-base font-normal h-10"
              :aria-label="`Back to main menu from ${currentView}`"
            >
            ...
            <DialogClose
              class="ring-offset-background focus:ring-ring data-[state=open]:bg-secondary w-11 h-10 flex items-center justify-center rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none"
          ...
          <div class="relative h-full" role="navigation" aria-label="Mobile navigation">
            <!-- Main View -->
            <Transition
              enter-active-class="transition-transform duration-300 ease-out absolute top-0 left-0 w-full"
              enter-from-class="-translate-x-full"
              enter-to-class="translate-x-0"
              leave-active-class="transition-transform duration-300 ease-out absolute top-0 left-0 w-full"
            ...
            >
              <div v-if="currentView === 'main'">
                ...
                <!-- Appearance Settings -->
                <div class="mt-4 pt-6 border-t px-6">
                  ...
                  <div>
                    ...
                    <ToggleGroup v-model="mode" type="single" variant="outline" class="w-full">
                      <ToggleGroupItem
                        ...
                        class="flex-1 font-normal gap-0 h-12"
                      >
            ...
            <!-- Submenu Views (dynamically rendered for items with children) -->
            <Transition
              enter-active-class="transition-transform duration-300 ease-out absolute top-0 left-0 w-full"
              enter-from-class="translate-x-full"
              enter-to-class="translate-x-0"
            ...
            >
              <div
                v-if="currentView !== 'main' && currentSubmenuItems.length > 0"
                class="flex flex-col gap-1 px-2 pt-3"
              >
                <label
                  class="text-sm font-normal text-muted-foreground mb-2 block px-4 capitalize pt-4"
                  >{{ currentView }}</label
                >
                <RouterLink
